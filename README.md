...
Demo已经部署到线上，  
地址 http://shiro.itboy.net

管理员帐号：admin  

密码：sojson.com 若密码错误，则用sojson。
...
“去其糟粕取其精华”。如果觉得写的好的地方就给个赞，写的不好的地方，也请多多包涵和 PR。
...
管理员帐号：admin
密码：sojson
ps:定时任务的 sql 会把密码改变为 `sojson.com` 
...
记住我登录的信息配置。与上面配置同理，可以在同个 一级域名  下的所有域名都可以获取到登录的信息。
...
```
若是仍不能解决问题，请在官方群（群号：259217951）内搜索“jstl” 如图下载依赖包。
